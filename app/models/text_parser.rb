class TextParser
  ...
  def self.parse_address(text)
    patterns = [["Strip fractions", /^(\d+ )(\d\/\d )?/, '\1'],
      ...
      ["Strip everything after the street designation", /^(.*? )(ave|avenue|blvd|boulevard|cir|circle|court|ct|c\.t|dr|drive|freeway|frwy|highway|hwy|lane|ln|mews|parkway|place|pl|pike|pkwy|rd|road|st|street|trail|trl|way)\b.*?$/i, '\1']]
    result = text
